...
export default class OpengraphReactComponent extends Component {
  ...
  componentDidMount() {
    if (this.props.dontMakeCall) {
      ...
      let url = `https://opengraph.io/api/1.1/site/http%3A%2F%2F${site}?accept_lang=${acceptLang}&app_id=${appId}`;
  ...
  renderStarsForRating = rating => {
    ...
    for (let i = 0; i < bestRating; i++) {
      if (i < ratingValue) {
        starArr.push(
          <i key={`star-${i}`} className={'coloredStar'}>
            ★
          </i>,
        );
      } else {
    ...
    return (
      <div className="outerWrapperSmall">
        ...
        <div className={'textWrapperSmall'}>
          ...
          <div className={'titleWrapper'}>{this.renderPrice(goodOffer, resultsToUse)}</div>
